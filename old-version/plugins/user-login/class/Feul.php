...
margin:5px 0;
			...
			Для просмотра этой страницы вы должны зарегистрироваться.</div>';	
			...
			{
				echo '<div class="updated">Предварительные настройки успешно созданы.</div>';
			}
		}
		...
		if(!file_exists(SITEUSERSPATH))
		{
			...
			if($create_feul_path)
			{
				echo '<div class="updated">Папка data/site-users успешно создана.</div>';
			}
			else
			{
				echo '<div class="error"><strong>Папка data/site-users не создана!</strong><br/>Вы можете создать этот каталог сами, в противном случае плагин работать не будет.</div>';
			}
		}
	}
	...
	*/  
	public function createDB()
	{
		...
		catch(PDOException $e)
		{
			if($this->Errors == 'On')
			{	
				echo '<div class="error"><strong>Не удалось создать базу данных:</strong> '.$e->getMessage().'</div>';
			}
		}
	}
	...
	*/  
	public function createDBTable()
	{
		...
		catch(PDOException $e)
		{
			if($this->Errors == 'On')
			{
				echo '<div class="error"><strong>Не удалось создать таблицу:</strong> '.$e->getMessage().'</div>';
			}
		}
	}
    ...
    */  
	public function getData($field)
	{
		...
		{
			return 'Ошибка: XML поле не существует.';
		}
	}
    ...
    */  
	public function processEditUser($name=null, $password=null, $email=null, $change_pass=null, $change_name=null)
	{
		...
		// create user xml file - This coding was mostly taken from the 'settings.php' page..
			...
			{
				print '<div class="updated">Пользователь отредактирован</div>';
			}
	}
