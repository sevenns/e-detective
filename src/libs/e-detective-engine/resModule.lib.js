...
var Tooltip=function(_text,_linked)
{
	...
	var that=
	{
		...
		show:function()
		{
			element=$("<div class='tooltip'>"+text+"</div>");
			$("body").append(element);
			element.css({opacity:1, display:"none"}).fadeIn(0);
			if(linked) 
			{
...
/*	var resizeExpPictures=function()
	...
	$(document).ready(resizeExpPictures);
	$("#"+params.story+" .mainPicture").click(resizeExpPictures);
	...
	var solveNow=function()
	{
		...
		if(statements.length>0)
		{
			...
			{
				case 0:
					solveWindow.prepend('<p class="successful-task">Улики интерпретированы верно, задача решена верно.</p>');
					break;
				case 1:
					solveWindow.prepend('<p class="not-solved-task">Улики интерпретированы неверно, задача решена неверно.</p>');
					break;
				case 2:
					solveWindow.prepend('<p class="semi-solved-task">Улики интерпретированы неверно, но задача решена верно.</p>');
					break;
				case 3:
					solveWindow.prepend('<p class="semi-solved-task">Вы нашли не все улики, но задача решена верно.</p>');
					break;
				case 4:
					solveWindow.prepend('<p class="not-solved-task">Вы нашли не все улики, задача решена неверно.</p>');
					break;
			}
		}
		else solveWindow.append('<p class="not-solved-task">Вы не выбрали ни одного высказывания. Я не умею так расследовать.</p>');
